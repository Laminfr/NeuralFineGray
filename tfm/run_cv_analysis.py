#!/usr/bin/env python
"""
Unified Cross-Validation Analysis Pipeline for Embedding Methods

This script orchestrates:
1. 5-Fold Cross-Validation
2. Hyperparameter Tuning (Random Search) within each fold
3. Embeddings generated dynamically inside folds (transductive setup)
...
# Separate hyperparameter grid for DeepSurv with embeddings
# Lower learning rates for stability with high-dimensional embeddings
HYPER_GRIDS_DEEP = {
    'DeepSurv': {
        'layers': [[100], [100, 100], [256, 128]],
...
# Models that should use PCA compression for embeddings
...
def run_experiment(model_name, mode, embedding_method, dataset='METABRIC', 
                   results_dir=None, n_iter=20, verbose=True):
    """
    ...
    Args:
        ...
        embedding_method: 'tabicl' or 'tarte'
    ...
    try:
        ...
        if dataset == 'SEER':
            effective_n_iter = min(n_iter, 20)
        
        if mode == 'raw':
            # Standard experiment without embeddings
            ...
            # Embedding-enhanced experiment
            use_pca = model_name in TREE_MODELS
            EmbeddingExperimentClass = EMBEDDING_EXPERIMENT_CLASSES[embedding_method]
            
            # Build kwargs based on embedding method
            exp_kwargs = {
            ...
            # Add method-specific mode parameter
            if embedding_method == 'tabicl':
        ...
        if predictions is not None:
            ...
            if verbose:
                print(f"  → {model_name} ({mode}): C-Index={metrics.get('c_index_q50', 0):.4f}, IBS={metrics.get('ibs', 0):.4f}")
        else:
...
def main(embedding_method, dataset='METABRIC', n_iter=None):
    """Main entry point."""
    
    # Use default iterations if not specified
    if n_iter is None:
        n_iter = DEFAULT_N_ITER.get(embedding_method, 20)
    
    # Set results directory
    results_dir = PROJECT_ROOT / f'results/{embedding_method}/cv_{dataset.lower()}'
    results_dir.mkdir(parents=True, exist_ok=True)
    
    method_upper = embedding_method.upper()
    print("=" * 60)
    print(f"CROSS-VALIDATION ANALYSIS: {method_upper} on {dataset}")
    print("=" * 60)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Run Cross-Validation Analysis with Embedding Methods',
