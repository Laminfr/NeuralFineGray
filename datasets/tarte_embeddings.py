...
def _encode_categorical_features(X_train: np.ndarray, X_val: np.ndarray, X_test: np.ndarray, X_dev: np.ndarray=None,
    ...
    if not cat_cols:
        ...
        X_dev = X_dev.astype(float) if X_dev is not None else None
        return X_train, X_val, X_test, X_dev, feature_names
    ...
    return X_train_enc, X_val_enc, X_test_enc, X_dev_enc, new_feature_names


def _apply_pca_compression(X_train_emb: np.ndarray, X_val_emb: np.ndarray, X_test_emb: np.ndarray, X_dev_emb: np.ndarray=None,
...
def apply_tarte_embedding(
    ...
    if pca_for_trees and use_deep_embeddings:
        X_train_emb, X_val_emb, X_test_emb, X_dev_emb, _ = _apply_pca_compression(
            X_train_emb, X_val_emb, X_test_emb, X_dev_emb,
    ...
    if concat_with_raw:
        ...
        X_train_raw_enc, X_val_raw_enc, X_test_raw_enc, X_dev_raw_enc, _ = _encode_categorical_features(
            X_train, X_val, X_test, X_dev, feature_names, verbose=verbose
        )
    ...
    return X_train_final, X_val_final, X_test_final, X_dev_final