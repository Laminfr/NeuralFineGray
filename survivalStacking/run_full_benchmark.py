...
def run_survstack_fold(
    ...
    metrics = compute_survival_metrics(
        T_test, E_test, survival_matrix, eval_times,
...
def run_coxph_fold(
    ...
    try:
        ...
        # IBS
        ibs = model.ibs(X_train_df, T_train_s, E_train_s, X_test_df, T_test_s, E_test_s, all_times)
        metrics['ibs'] = ibs
...
def run_deepsurv_fold(
    ...
    try:
        ...
        metrics['ibs'] = integrated_brier_score(
            T_test, E_test, survival_matrix, all_times,
            T_train=T_train, E_train=E_train
        )
...
def run_full_cv(
    ...
    for method in methods:
        if fold_results[method]:
            ...
            results[method] = {
                'mean': {k: np.mean([f[k] for f in fold_results[method]]) for k in metric_keys},
                'std': {k: np.std([f[k] for f in fold_results[method]]) for k in metric_keys},
...
def main():
    ...
    parser.add_argument('--n_intervals', type=int, default=20, help='Number of time intervals')
    ...
    for dataset in datasets:
        ...
        results = run_full_cv(
            ...
            run_survstack_emb=not args.skip_tabicl,
